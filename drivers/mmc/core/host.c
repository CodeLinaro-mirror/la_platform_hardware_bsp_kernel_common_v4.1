...
int mmc_of_parse(struct mmc_host *host)
{
	...
	if (of_find_property(np, "card-reset-gpios", NULL)) {
		...
		for (i = 0; i < ARRAY_SIZE(host->card_reset_gpios); i++) {
			gpd = devm_gpiod_get_index(host->parent,
						   "card-reset", i);
			if (IS_ERR(gpd))
				break;
		...
		gpd = devm_gpiod_get_index(host->parent, "card-reset",
	...
	host->card_clk = devm_clk_get(host->parent, "card_ext_clock");
	...
	host->card_regulator = devm_regulator_get(host->parent,
						  "card-external-vcc");
